...
#
bf1 = [g[string(v), :name] for v in 5:14]
bf2 = [g[string(v), :name] for v in 12:17]
    ...
    for n in all_neighbors(g, common)
        if (n ∉ bf1 || n ∉ bf2)
            if get_prop(g, Edge(n, common), :switch) == 1
                println(string(g[n, :name], "-", g[common, :name], " crosses and has a switch"))
            else
                println(string(g[n, :name], "-", g[common, :name], " crosses and doesn't have a switch"))
            end
        end
    end
end
