        ...
        let network = deepcopy(network)
            ...
            if length(islands) == 1
                ...
                if splits
                    ...
                    temp_p = sum(
                        part.rest_power + part.rest_power * s_time * (1 - γ * length(part.leaf_nodes)) for part in temp_parts
                    )
                    if temp_p <= best_p
...
for comb_idx in combinations(1:length(parts), 2)
    ...
    if length(overlapping) > 0
        ...
        parts[comb_idx], split_edge =
            ...
            splitting_times[comb_idx] .+= get_min_cutting_time(network[split_edge...])
