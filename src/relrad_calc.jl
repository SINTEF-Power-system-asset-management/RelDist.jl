...
function get_switch(mpc::Case, e::Branch)
    ...
    if !switch.closed # When trying to open an open switch, that doesn't take any time
        # BEWARE: if implementing closing of open switches
        return Switch(switch.f_bus, switch.t_bus, 0.0, 0.0)
    end
...
function traverse_and_get_sectioning_time(network::RadialPowerGraph, e::Branch,
    ...
    if e.src ∈ switch_buses
        ...
        # There is a logical error here, which is that if the source edge exists on the
        # list of switches, but the switch on the other side is faster, we will still
        # find the faster switch on the wrong side.
        append!(seen, s)
    else
