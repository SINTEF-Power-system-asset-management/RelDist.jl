...
function fix_parts_with_same_vertices!(o::Overlapping)
   if !not_all_served(o.old_p)
       # The old part could supply everything. It probably is the
       # main feeder or somehting. Let us not bother to add the one
       # we just checked
       o.add_new_part = false
    else
        find_reconfiguration_switches!(o)
...
function find_reconfiguration_switches!(o::Overlapping)
    ...
    if best_P == 0
        ...
        for (part, split) in splits
            update_part_after_split!(getfield(o, part), o.g, split, split_switch)
        end
        return 
    end
end
