...
function f_bkk_lines(mpc, network, dfs)
    ...
    for row in collect(eachrow(network["lines"]))
        ...
        reldata_entry = [size(mpc.branch)[1]+1, 
                        from, 
                        to, 
                        repair_time, 
...
function f_bkk_switch(mpc, network, dfs)
    ...
    for row in collect(eachrow(network["switches"]))
        ...
        switches_entry = [from, 
                          to, 
                          breaker, 
                          if row["normalOpen"]==0 "True" else "False" end]
...
function f_bkk_transformers(mpc, network, dfs)
    ...
    for row in collect(eachrow(network["transformers"]["trafo_2w"]["h"]))
        ...
        terminal = trafo_l[(trafo_l[:,:PowerTransformer].==transformer),:]["ConnectivityNode"][1]
        trafos_entry = [row["PowerTransformer"], 
       ...
       # end
        lines_entry = [row["ConnectivityNode"], terminal, 0, 0, 0, 0, 0, 0, 0, 0, transformer]
        reldata_entry = [size(mpc.branch)[1]+1, row["ConnectivityNode"], terminal, 0, 0, 0, 0, 0.02, row["ratedS"]]
        push!(mpc.branch, lines_entry)
        push!(mpc.reldata, reldata_entry)
    end
    ...
    for row in collect(eachrow(network["transformers"]["trafo_3w"]["m"]))
        # if row["transformer_type"]=="secondary"
        transformer = row["PowerTransformer"]
        terminal = trafo_h[(trafo_h[:,:PowerTransformer].==transformer),:]["ConnectivityNode"][1]
        trafos_entry = [row["PowerTransformer"], 
                        terminal,
                        row["ConnectivityNode"], 
                        row["ratedS"]]
...
function f_bkk_process_slack(mpc, dfs)
    mpc.transformer = leftjoin(mpc.transformer, dfs["ConnectivityNodeVoltage"], on=[(:f_bus, :ConnectivityNode)])
    filtered_transformer = mpc.transformer[mpc.transformer.voltage.==maximum(mpc.transformer.voltage),:]
...
function read_xmls(folder_name, xml_types_file="databases/xml_types.json", drop_duplicates=true)
        ...
        for name in names(dfs[key])
            ...
            try
                if col_type == "reference"
                    try
                        dfs[key][name] = String.(reduce(hcat,split.(coalesce.(dfs[key][name],"_ "),'_')))[end,:] # coalesce substitutes missing values with a default
                    catch
                        continue
                    end
                elseif col_type == "float64" || col_type ==  "int64"
                    try
                        dfs[key][name] = parse.(str2datatype[col_type],replace.(dfs[key][name],(","=>".")))
                    catch
                        continue
                    end
                elseif col_type == "bool"
                    dfs[key][name] = convert(Array{str2datatype[col_type],1},dfs[key][name].=="true")
                else
                    try
                ...
                println("Failed to set type ({col_type}) of column ({column}) of DataFrame ({table})")    
            end
        end
    end
end
...
function terminalData(dfs)
    ...
    select!(t1, "ConductingEquipment" => "ConductingEquipment", "ConnectivityNode" => "ConnectivityNode1")
    select!(t2, "ConductingEquipment" => "ConductingEquipment", "ConnectivityNode" => "ConnectivityNode2")
    select!(t3, "ConductingEquipment" => "ConductingEquipment", "ConnectivityNode" => "ConnectivityNode3")
    return leftjoin(leftjoin(t1, t2, on="ConductingEquipment"), t3, on="ConductingEquipment")
end

function switch(dfs, bus_idx)
    from_bus, to_bus = terminals(dfs, bus_idx)
    switches = innerjoin(innerjoin(vcat(dfs["Breaker"],
                                dfs["Disconnector"],
                                dfs["Fuse"],
                                dfs["GroundDisconnector"],
                                dfs["Jumper"],
                                dfs["LoadBreakSwitch"],
                                cols=:union), from_bus,on=[(:mRID, :ConductingEquipment)],makeunique=true),
                                to_bus,on=[(:mRID, :ConductingEquipment)],makeunique=true) 
    return switches
end


function equipmentContainerWithVoltage(dfs)
    bay = innerjoin(dfs["Bay"], select(dfs["VoltageLevel"], "mRID"=>"VoltageLevel", "BaseVoltage"=>"BaseVoltage", "Substation"=>"Substation"), on="VoltageLevel")
    voltage_level = leftjoin(dfs["VoltageLevel"], dfs["BaseVoltage"], on=:BaseVoltage=>:mRID)
...
function switchData(dfs)
    result = leftjoin(leftjoin(innerjoin(dfs["Switch"], dfs["TerminalData"], on=:mRID=>:ConductingEquipment), 
                        select(dfs["BaseVoltage"], "mRID"=>"mRID", "nominalVoltage"=>"voltage"), on=:BaseVoltage => :mRID), 
                        select(dfs["EquipmentContainerWithVoltage"], "nominalVoltage"=>"equipmentVoltage", "mRID"=>"mRID"), on=:EquipmentContainer => :mRID)
...
function powerTransformerEndData(dfs)
    result = leftjoin(leftjoin(dfs["PowerTransformerEnd"], dfs["Terminal"], on=:Terminal=>:mRID),
                select(dfs["BaseVoltage"],"mRID"=>"mRID", "nominalVoltage"=>"voltage"), on=:BaseVoltage=>:mRID)
...
function busbarSectionData(dfs)
    result = leftjoin(leftjoin(innerjoin(dfs["BusbarSection"], dfs["TerminalData"], on=:mRID=>:ConductingEquipment),
                        select(dfs["BaseVoltage"], "mRID"=>"mRID", "nominalVoltage"=>"voltage"), on=:BaseVoltage => :mRID), 
                        select(dfs["EquipmentContainerWithVoltage"], "nominalVoltage"=>"equipmentVoltage", "mRID"=>"mRID"), on=:EquipmentContainer => :mRID)
    result = replace_missing(result, "voltage", "equipmentVoltage")
...
function energyConsumerData(dfs)
    result = leftjoin(leftjoin(innerjoin(dfs["EnergyConsumer"], dfs["TerminalData"], on=:mRID=>:ConductingEquipment),
                        select(dfs["BaseVoltage"], "mRID"=>"mRID", "nominalVoltage"=>"voltage"), on=:BaseVoltage => :mRID), 
                        select(dfs["EquipmentContainerWithVoltage"], "nominalVoltage"=>"equipmentVoltage", "mRID"=>"mRID"), on=:EquipmentContainer => :mRID)
    # result.voltage .= replace.(result.baseVoltage, missing => result["equipmentVoltage"])
    result = replace_missing(result, "voltage", "equipmentVoltage")
...
function acLineSegmentData(dfs)
    result = leftjoin(leftjoin(leftjoin(innerjoin(dfs["ACLineSegment"], dfs["TerminalData"], on=:mRID=>:ConductingEquipment), 
                        select(dfs["PSRType"], "mRID"=>"mRID", "name"=>"PSRTypeName"), on=:PSRType => :mRID),
                        select(dfs["BaseVoltage"], "mRID"=>"mRID", "nominalVoltage"=>"voltage"), on=:BaseVoltage => :mRID), 
                        select(dfs["EquipmentContainerWithVoltage"], "nominalVoltage"=>"equipmentVoltage", "mRID"=>"mRID"), on=:EquipmentContainer => :mRID)
    #result.voltage .= replace.(result.baseVoltage, missing => result["equipmentVoltage"])
    result = replace_missing(result, "voltage", "equipmentVoltage")
...
function connectivityNodeVoltage(dfs)
    grouped_result = groupby(leftjoin(select(dfs["Terminal"], Not(:sequenceNumber)), 
                    dfs["ConductingEquipment"], on=:ConductingEquipment=>:mRID), :ConnectivityNode)
    result = combine(grouped_result, (voltage = :voltage=>median))
    result = replace_missing(result, dfs["PowerTransformerEndData"], "voltage", "ConnectivityNode")
...
function add_lines(dfs, bus_idx)
    from_bus, to_bus = terminals(dfs, bus_idx)
    lines = leftjoin(innerjoin(innerjoin(dfs["ACLineSegment"],from_bus,on=[(:mRID, :ConductingEquipment)],makeunique=true),
        to_bus,on=[(:mRID, :ConductingEquipment)],makeunique=true),
        dfs["PSRType"],on=[(:PSRType,:mRID)],makeunique=true)
    return lines
end
...
function trafo_2w_ends(dfs, bus_idx)
    power_transformer_end = leftjoin(leftjoin(dfs["PowerTransformerEnd"],
        select(dfs["Terminal"],[:mRID,:ConnectivityNode]),on=[(:Terminal,:mRID)]),
        bus_idx,on=:ConnectivityNode)
    ...
    for transformer in trafo_2w
        if i == 0
            ...
            lv_end_2w = vcat(lv_end_2w, DataFrame(trafo_end_groups[(PowerTransformer=transformer,)][1,:]))
            hv_end_2w = vcat(hv_end_2w, DataFrame(trafo_end_groups[(PowerTransformer=transformer,)][2,:]))
        end   
        i += 1
    end
...
function trafo_3w_ends(dfs, bus_idx)
    ...
    for transformer in trafo_3w
        if i == 0
            ...
            lv_end_3w = vcat(lv_end_3w, DataFrame(trafo_end_groups[(PowerTransformer=transformer,)][1,:]))
            mv_end_3w = vcat(mv_end_3w, DataFrame(trafo_end_groups[(PowerTransformer=transformer,)][2,:]))
            hv_end_3w = vcat(hv_end_3w, DataFrame(trafo_end_groups[(PowerTransformer=transformer,)][3,:]))
        end   
        i += 1
    end
...
function add_loads(dfs, bus_idx)
    from_bus, to_bus = terminals(dfs, bus_idx)
    loads = innerjoin(dfs["BusbarSection"], from_bus, on=[(:mRID, :ConductingEquipment)],makeunique=true)
    return loads
end


function export_buses(network)
    df = DataFrame(ID=Int[], type=Int[], Pd=Float64[], Qd=Float64[], Gs=Float64[], Bs=Float64[],
        area_num=Int[], Vm=Float64[], Va=Float64[], baseKV=Float64[], zone=Int, max_Vm=Float64[], min_Vm=Float64[])
    rw = Array{Any,1}()
...
IC, ICt, L, edge_pos = relrad_calc(interruption, cost_functions, meta_network, filtered_branches)
